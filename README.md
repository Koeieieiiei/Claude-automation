...
## 🧪 ระบบทำข้อสอบออนไลน์ (branch `exam-demo` — ยังเป็นแบบจำลอง ไม่ได้ขึ้นเว็บจริง)
...
**ก่อนขึ้นเว็บจริง ต้องทำเพิ่ม (จงใจยังไม่ทำในแบบจำลอง):**
- [ ] ย้ายที่เก็บข้อมูลจากไฟล์ `data/exam/` → Supabase (โครงตารางเตรียมแล้วที่ [supabase/exam-schema.sql](supabase/exam-schema.sql)) — บน Vercel ไฟล์ในเครื่องหายทุก deploy
- [ ] เพิ่ม `product_id` ในตาราง orders แล้วต่อ `findEntitlement()` ให้เช็คผู้ซื้อจริง (ตอนนี้เช็คเฉพาะผู้ซื้อจำลอง) — เช็คชื่อ+อีเมลกับคอลัมน์ first_name/email ที่มีอยู่แล้ว
- [ ] อัปโหลดรูปหน้าโจทย์ (`assets/exam-pages/`) + เฉลย (`data/exam/answer-key.json`) ขึ้น Supabase Storage
- [ ] ตัดสินใจเรื่อง "ประชากรจำลอง 612 คน" — ช่วงแรกผู้สอบจริงน้อย กราฟ/อันดับต้องอิงอะไร (ใช้ต่อแบบติดป้ายว่าเป็นสถิติจำลอง หรือรอผู้สอบจริงสะสม)
- [ ] (ถ้าต้องการ) โฆษณาสิทธิ์ทำข้อสอบออนไลน์บนการ์ดสินค้า/FAQ — ตอนนี้หน้าขายยังไม่พูดถึง

หมายเหตุ: การซื้อ/อีเมลส่งไฟล์/หน้า success **ไม่ต้องแก้อะไรเลย** — เจ้าของตัดสินใจให้ส่งไฟล์ครบตามปกติ
การทำข้อสอบออนไลน์เป็นสิทธิ์เสริม (ระบบจะเตือนในหน้ากติกาว่าไม่ควรเปิดเฉลยก่อนสอบ เพื่อให้ผลแม่น)
